...
@Repository("zhifubaoDao")
public class AlipayDao extends BaseDao {
	public AlipayDao() {
		super();
		// clazz对应key名中的Staff
		this.clazz = "thirdParty:alipay";
		// hashTag对应key名中的｛staff｝
		this.hashTag = "thirdParty";
		// 设置uid为hash_key
		this.idDescriptor = createFieldDescriptor("alipayOpenId", FieldType.TYPE_EQUAL);
		// 设置需要搜索的字段，priority为字段名，通过FieldType设置搜索模式，EQUAL对应相等匹配查询，MATCH对应模糊匹配查询，RANGE对应范围匹配查询
		addFieldDescriptor(createFieldDescriptor("alipayOpenId", FieldType.TYPE_EQUAL));
